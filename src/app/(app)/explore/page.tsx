...
const MOCK_FEED = [
  { id: "1", format: "gossip", emoji: "🗣️", color: "#ec4899", title: "AP Bio: Cell Division Drama", creator: "Emma K.", avatar: "E", subject: "AP Biology", preview: "Mia: omg so mitosis literally divides the cell into TWO identical copies...", likes: 234, views: 1200 },
  { id: "2", format: "podcast", emoji: "🎙️", color: "#f59e0b", title: "Calculus Deep Dive: Derivatives", creator: "Jake M.", avatar: "J", subject: "AP Calculus", preview: "Alex: So Jordan, today we're talking about derivatives...", likes: 156, views: 890 },
  ...
  { id: "5", format: "gossip", emoji: "🗣️", color: "#ec4899", title: "WW2 Drama: The Real Story", creator: "Aisha B.", avatar: "A", subject: "AP History", preview: "Zoe: okay but did you hear what happened at Dunkirk??", likes: 445, views: 2100 },
];

const SUBJECTS = ["All", "AP Biology", "AP Chemistry", "AP Physics", "AP Psychology", "AP Calculus", "AP History", "College"];
    ...
    <div className="min-h-screen bg-zinc-950">
      <div className="px-4 pt-12 pb-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4">
          {SUBJECTS.map(s => (
            <button key={s} onClick={() => setFilter(s)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${filter === s ? "bg-purple-600 text-white" : "bg-zinc-800 text-zinc-400"}`}>{s}</button>
          ))}
        </div>
      </div>
      <div className="px-4 space-y-4 pb-24">
        {feed.map(item => (
          <div key={item.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
            <div className="p-4">
              ...
              <div className="p-3 rounded-xl text-xs text-zinc-400 leading-relaxed" style={{ backgroundColor: `${item.color}08`, border: `1px solid ${item.color}20` }}>"{item.preview}"</div>
            </div>
        ...
        <div className="text-center py-4"><p className="text-zinc-600 text-sm">You've seen it all! 🎉</p></div>
      </div>
    </div>
  );
}
