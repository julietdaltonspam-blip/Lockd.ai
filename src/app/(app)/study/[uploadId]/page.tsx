...
export default function StudyPage({ params }: { params: Promise<{ uploadId: string }> }) {
  ...
  const [format, setFormat] = useState<OutputFormat | null>(
    (searchParams.get("format") as OutputFormat) || null
  );
  ...
  async function generateContent(selectedFormat: OutputFormat) {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ content, format: selectedFormat, uploadId }),
      });
  ...
  function tryAnotherFormat() { setFormat(null); setGeneratedData(null); setError(""); }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 px-4">
        <div className="relative mb-8">
          ...
          <div className="w-24 h-24 rounded-full bg-purple-900/50 flex items-center justify-center relative animate-pulse-glow">
            <span className="text-4xl">✨</span>
          </div>
        </div>
        <p className="text-white font-bold mb-2">Generating {format} mode...</p>
        <p className="text-zinc-400 text-sm">Claude is working its magic</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950">
        <div className="px-4 pt-12">
          ...
          {!content ? (
            <div className="text-center py-12"><p className="text-zinc-400">No content found. Please upload something first.</p></div>
          ) : (
            <FormatPicker onSelect={generateContent} />
          )}
        </div>
      </div>
    );
  ...
  const sharedProps = { onTryAnother: tryAnotherFormat, content, uploadId };
