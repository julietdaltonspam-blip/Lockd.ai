...
const FORMAT_SUGGESTIONS = [
  ...
  { format: "podcast", label: "Podcast Mode", emoji: "🎙️", color: "#f59e0b" },
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="px-4 mb-4">
        <div className="flex gap-3">
          <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl p-3 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-orange-950/50 flex items-center justify-center">
          ...
          <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl p-3 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-purple-950/50 flex items-center justify-center">
              <Zap size={20} className="text-purple-400" />
            </div>
            <div>
              <p className="text-xl font-black text-purple-400">{xp}</p>
              <p className="text-zinc-500 text-xs">{level}</p>
            </div>
          </div>
          <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl p-3 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-950/50 flex items-center justify-center">
              <Trophy size={20} className="text-amber-400" />
            </div>
            <div>
              <p className="text-xl font-black text-amber-400">#</p>
              <p className="text-zinc-500 text-xs">leaderboard</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {recentUploads.length > 0 ? (
        <section className="px-4 mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/library" className="text-purple-400 text-xs font-medium flex items-center gap-1">
          ...
          <div className="space-y-2">
            {recentUploads.map(upload => (
              <Link
                key={upload.id}
                href={`/study/${upload.id}`}
                className="flex items-center gap-3 p-3.5 bg-zinc-900 border border-zinc-800 rounded-2xl hover:border-zinc-700 transition-all active:scale-[0.99]"
              >
                <div className="w-10 h-10 rounded-xl bg-purple-950/50 flex items-center justify-center">
        ...
        <section className="px-4 mb-6">
          <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-2xl text-center">
            ...
            <Link
              href="/upload"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl btn-purple text-white text-sm font-bold"
            >
              <Plus size={16} />
              Upload something
            </Link>
        ...
        <section className="px-4 mb-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
            {FORMAT_SUGGESTIONS.map(f => (
              <Link
                key={f.format}
                href={`/study/${recentUploads[0]?.id}?format=${f.format}`}
          ...
          {WEEKLY_CHALLENGES.map(c => (
            <div key={c.title} className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className="text-xs font-bold text-purple-400 bg-purple-950/50 px-2 py-0.5 rounded-full">
                  +{c.xp} XP
                </span>
              </div>
      ...
      <section className="px-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-bold text-sm text-zinc-300 uppercase tracking-wide">
            <TrendingUp size={14} className="inline mr-1" />
            Trending from students
          </h2>
          <Link href="/explore" className="text-purple-400 text-xs font-medium flex items-center gap-1">
            See all <ChevronRight size={12} />
          </Link>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
          {["AP Bio Gossip 🗣️", "Calc Podcast 🎙️", "Psych Slides 📱"].map((item, i) => (
            <Link
              key={item}
              href="/explore"
              className="flex-shrink-0 w-40 h-28 rounded-2xl bg-zinc-900 border border-zinc-800 flex flex-col justify-end p-3 hover:border-zinc-700 transition-all overflow-hidden relative"
            >
              <div className="absolute inset-0 opacity-20" style={{
                background: `linear-gradient(135deg, ${["#ec4899", "#f59e0b", "#3b82f6"][i]}, transparent)`,
              }} />
