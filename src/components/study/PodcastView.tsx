...
const AMBIENTS: { id: AmbientMode; label: string; emoji: string; class: string }[] = [
  { id: "cafe", label: "Rainy Café", emoji: "☕", class: "ambient-cafe" },
  { id: "fireplace", label: "Fireplace", emoji: "🔥", class: "ambient-fireplace" },
  { id: "city", label: "Lo-fi City", emoji: "🌙", class: "ambient-city" },
  { id: "library", label: "Library", emoji: "📚", class: "ambient-library" },
  { id: "rain", label: "Rain", emoji: "🌧️", class: "ambient-rain" },
];
...
export function PodcastView({ data, onTryAnother }: PodcastViewProps) {
  ...
  return (
    <div className={`min-h-screen pb-32 relative transition-all duration-700 ${currentAmbient.class}`}>
      <div className="sticky top-0 z-30 glass border-b border-zinc-800/50 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2"><span className="text-lg">🎙️</span><span className="font-bold text-amber-400">Podcast Mode</span></div>
        <button className="text-zinc-400"><Share2 size={18} /></button>
      </div>
      <div className="px-4 py-3 flex gap-2 overflow-x-auto">
        {AMBIENTS.map(a => (
          <button key={a.id} onClick={() => setAmbient(a.id)} className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
            ambient === a.id ? "bg-amber-900/60 border border-amber-600/60 text-amber-300" : "bg-zinc-800/50 border border-zinc-700 text-zinc-400"
          }`}>
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-zinc-900/80 border border-zinc-700/50">
          <button onClick={() => setPlaying(!playing)} className="w-12 h-12 rounded-full bg-amber-600 flex items-center justify-center flex-shrink-0">
          ...
          return (
            <div key={i}>
              ...
              <div className={`flex gap-3 ${isAlex ? "" : "flex-row-reverse"}`}>
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 mt-1" style={{ backgroundColor: isAlex ? "#f59e0b20" : "#a855f720" }}>
